**Note:** this repository contains 2 branches, one containing the starter code written in Typescript, one containing the starter code written in Java. Everyone is expected to complete all of the tasks twice, once for each language. In order to access each branch, you need to use the git checkout command. See Recitation 1 for an example. Recitation 2 will further cover using Git. 
...
For your Java branch, please include a launch.json file containing some example arguments to run your CLI with. See Recitation 1 for an example of how to provide multiple arguments to your CLI in a launch.json file.  
...
* [ ] 9: The build passes on GitHub Actions, including the automated style checks. 4.5 points per language. 
* [ ] 1: Submits a launch.json file containing arguments to run the Java CLI with. 
